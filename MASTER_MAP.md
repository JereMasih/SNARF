...
Desde el 2026-07-29 (ADR 0047 — Mark II): una conversación puede pertenecer formalmente a un Proyecto (`project_id` persistente en `data/conversation_projects.json`, no un parámetro por mensaje) — mientras dure la asociación, el prompt del proyecto se aplica automáticamente en todos sus turnos, sin reescribir nunca el historial ya generado bajo una asignación anterior. Tools `project_assign_conversation`/`project_unassign_conversation`/`project_list_conversations` (no gateadas). El modal chico de detalle se retira: entrar a un proyecto escala la barra lateral (solo sus conversaciones) y muestra un "home" en el área de chat — estadísticas reales (`file_count` excluyendo subcarpetas, `pending_task_count`), resumen generado por Snarf (`cached_summary`, mismo patrón que el digest de Gmail), prompt con contador de caracteres (`PROJECT_PROMPT_MAX_LENGTH = 4000`).

Desde el 2026-07-30 (ADR 0056, Parte 1 de un plan de eficiencia de costo de 4 partes): nuevo `snarf/voice/` — capa de voz con proveedores intercambiables detrás de un router (`STTProvider`/`TTSProvider`, elegidos en `voice/config.yaml`, nunca en código). STT primario Groq (`whisper-large-v3-turbo`), fallback 100% local y gratis (`faster-whisper`) sin red o si Groq falla. TTS con tres tiers: "local" (Kokoro-FastAPI en Docker, CPU, gratis — nuevo default de toda conversación), "hosted" (stub deliberado, sin proveedor real todavía — sin evidencia de que "local" no alcance), "premium" (ElevenLabs, exclusivo, nunca usado sin pedirlo explícito). Separado de esto, la optimización de mayor impacto real del cambio: cada respuesta de Snarf ahora se separa en versión completa (a pantalla, `AnthropicLLM.generate()` → `Orchestrator.handle()` devuelven `LLMResponse(text, speech)`) y versión hablada breve (a voz, instruida por prompt con un marcador delimitado, <400 caracteres, nunca oculta un riesgo o dato faltante presente en la versión completa). La burbuja de cada respuesta sigue el canal de entrada: turno por voz sintetiza y muestra el resumen hablado solo (sin click, sin autoplay); turno por texto lo deja como botón manual; un botón separado y más discreto ("🔊 completa") ofrece siempre a pedido la respuesta larga entera en voz. `GROQ_API_KEY` real ya conseguida y verificada (6 audios reales transcriptos en rioplatense correcto). Partes 2 (router de modelos), 3 (ledger de costo + comando `snarf costo`) y 4 (portabilidad a VPS) del mismo plan, pendientes.
...
Desde el 2026-07-30 (ADR 0056): primer uso real de Docker en el proyecto (Colima como runtime en la Mac del fundador, sin Docker Desktop — mismo Docker CLI/API por debajo). `docker-compose.voice.yml` levanta el contenedor de Kokoro-FastAPI (tier "local" de TTS) — deliberado desde el día uno pensando en la futura migración a VPS (`VPS_MIGRATION.md`): el mismo contenedor corre igual ahí, sin cambios.
...
Desde el 2026-07-30 (ADR 0067 y 0068, cierre real de la "Parte 2" del plan de eficiencia de costo del ADR 0056 — resultó distinta de lo planeado originalmente): análisis contra `data/usage_log.jsonl` real mostró que el ruteo de modelos por Especialista ya estaba resuelto (Gmail digest, resumen de proyectos, visión de Drive y título de conversación ya en Haiku) — el 96% del gasto era el loop principal, con dos causas puntuales: un pedido sin tope de "mil correos" costando $1.09 en una sola llamada por quedar embebido y re-transmitirse turno a turno, y 4 llamadas cortadas por `MAX_OUTPUT_TOKENS`. `Orchestrator._bulk_read_gate()` (mismo protocolo `_pending()` de confirmación en dos pasos que las tools de alto impacto, pero por costo, no irreversibilidad — nunca un bloqueo permanente, confirmado se ejecuta la cantidad exacta pedida) y `_capped_for_replay()` (tope solo sobre lo que se re-transmite al LLM en turnos futuros, nunca sobre lo guardado/mostrado) resuelven eso. Investigación real de precios de Gemini/OpenAI/xAI/Llama mostró que ningún proveedor combina hoy "tan inteligente como Sonnet" con "notablemente más barato" — igual se construyó soporte multi-proveedor real (`OpenAICompatibleLLM` cubre OpenAI/xAI/Llama-vía-Groq con una sola clase, `GeminiLLM` aparte, ambas verificadas campo por campo contra los SDKs reales instalados) vía `snarf/runtime/llm_routing.py`: qué proveedor/modelo usa cada rol, configurable desde la interfaz, default = comportamiento de siempre. Los proveedores nuevos todavía no tienen credencial real cargada — código y tests completos, falta el smoke-test en vivo.
...
- **Construida el 2026-07-29 (ADR 0045, extendida el mismo día por ADR 0047 — Mark II):** la Capacidad "Proyectos" en sí — prompt de proyecto, tareas y notas propias, carpeta (y subcarpetas propuestas) en Drive, búsqueda semántica acotada al proyecto, barra lateral con switcher Conversaciones/Proyectos, y ahora conversaciones formalmente asociadas a un proyecto (prompt aplicado automáticamente en cada turno, sin reescribir historial al reasignar). Queda pendiente, sin fecha: migración de "Proyectos" externos (ChatGPT: Alimentación y Workout, High Value Men) hacia adentro de un Proyecto real de Snarf, y arquitectura de Especialistas por dominio corriendo dentro de un Proyecto puntual (ver Capabilities).
