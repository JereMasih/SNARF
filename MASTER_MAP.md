...
5. (siguiente) Migración de Snarf de la Mac del fundador a un VPS **Linux** (no Windows — todo el stack, incluido `ffmpeg`, corre nativo ahí, y sale más barato) — que Snarf esté disponible 24/7 sin depender de que la Mac esté prendida. El fundador reportó que la interfaz se siente lenta incluso antes de que existiera la indexación, accediendo desde el celular a través de algún servicio de túnel/acceso remoto a la Mac (no identificado todavía) — la migración a un VPS con hosting real es, con esa información, la causa raíz más probable, no solo una mejora.
...
- Migración de "Proyectos" externos (ChatGPT: Alimentación y Workout, High Value Men) + arquitectura de Especialistas por dominio dentro de Snarf.
- Visualización "cerebro de Snarf" estilo Jarvis, sobre el registro real del ítem 2 de la Fundación.
