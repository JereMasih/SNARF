...
Desde el 2026-07-29 (ADR 0041): `GoogleDrive`/`GoogleGmail`/`GoogleCalendar`/`GoogleYouTube` reintentan una vez con un cliente fresco ante un fallo transitorio de conexión (`retry_once_with_fresh_client`, solo en lecturas idempotentes) — corrige un fallo real visto en producción (`[SSL] record layer failure`) por la conexión cacheada como singleton en un proceso de larga vida. Gmail ahora muestra fecha real por mensaje, y el digest interpretado por el LLM viene acompañado de una referencia estructurada real (no inventada) de los mensajes que resume. Nuevo widget `usage`: consumo real por vendor (llamadas/tokens/caracteres/segundos) más el cupo real de la cuenta de ElevenLabs consultado en vivo — distinto del panel `cost`, que siempre fue una estimación en dólares, nunca un saldo real. Tamaños default de los widgets del dashboard recalibrados con evidencia real (los tamaños que el propio fundador ya había elegido a mano). Cuadro de texto de envío pasa de una sola línea a un textarea que crece.
...
- Migración de "Proyectos" externos (ChatGPT: Alimentación y Workout, High Value Men) + arquitectura de Especialistas por dominio dentro de Snarf. **Ampliado el 2026-07-29**: el fundador pidió explícitamente que Snarf tenga su propia versión de "Proyectos" al estilo Claude/ChatGPT — prompt de proyecto (instrucciones propias por proyecto), archivos subidos que Snarf mantenga visualmente accesibles pero alojados donde corresponda (siempre intentando Google Drive primero, usables vía el vectorizado ya existente), y que Snarf proponga una ubicación de carpeta/subcarpetas en Drive para organizar los archivos de cada proyecto nuevo. La barra lateral que hoy vive en el menú hamburguesa de mobile y en el modo enfoque del chat (ADR 0035) es el lugar ya identificado donde esta navegación va a vivir. Registrado, no construido — es una Capacidad nueva entera, con su propio ciclo de diseño pendiente.
