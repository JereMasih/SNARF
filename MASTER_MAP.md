...
Desde el 2026-07-30 (ADR 0056, Parte 1 de un plan de eficiencia de costo de 4 partes): nuevo `snarf/voice/` — capa de voz con proveedores intercambiables detrás de un router (`STTProvider`/`TTSProvider`, elegidos en `voice/config.yaml`, nunca en código). STT primario Groq (`whisper-large-v3-turbo`), fallback 100% local y gratis (`faster-whisper`) sin red o si Groq falla. TTS con tres tiers: "local" (Kokoro-FastAPI en Docker, CPU, gratis — nuevo default de toda conversación), "hosted" (stub deliberado, sin proveedor real todavía — sin evidencia de que "local" no alcance), "premium" (ElevenLabs, exclusivo, nunca usado sin pedirlo explícito). Separado de esto, la optimización de mayor impacto real del cambio: cada respuesta de Snarf ahora se separa en versión completa (a pantalla, `AnthropicLLM.generate()` → `Orchestrator.handle()` devuelven `LLMResponse(text, speech)`) y versión hablada breve (a voz, instruida por prompt con un marcador delimitado, <400 caracteres, nunca oculta un riesgo o dato faltante presente en la versión completa). Pendiente antes de cerrar esta parte: `GROQ_API_KEY` real del fundador, para verificar calidad de STT en español rioplatense con audios reales (criterio de aceptación explícito del plan). Partes 2 (router de modelos), 3 (ledger de costo + comando `snarf costo`) y 4 (portabilidad a VPS) del mismo plan, pendientes.
