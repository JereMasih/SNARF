...
Entregá siempre en español — el chat y el reporte guardado — salvo que quien pida la auditoría escriba el pedido en inglés, en cuyo caso entregá en inglés. Esto aplica a la tabla de veredictos, la tabla de exposición, y todas las secciones del reporte final. El razonamiento interno y las búsquedas pueden hacerse en cualquier idioma; lo que importa es el idioma del texto que ve el usuario.
...
Imprimí en el chat, en español (salvo pedido en inglés — ver "Idioma de entrega" arriba), y después guardá en `audits/os-audit-YYYY-MM-DD.md` (creá `audits/` si hace falta; es la única escritura de esta skill).

```
# Auditoría de OS — {fecha}

**Conocimiento vigente hasta: {la fecha de obsolescencia efectiva del Check 3, capa ingerida}**

| Check | Veredicto | Peor hallazgo |
|---|---|---|
| Integridad de routing | VERDE/AMARILLO/ROJO | ... |
| Veracidad de índices   | VERDE/AMARILLO/ROJO | ... |
| Frescura               | VERDE/AMARILLO/ROJO | ... |
| Bloat/duplicación      | VERDE/AMARILLO/ROJO | ... |
| Higiene                | VERDE/AMARILLO/ROJO | ... |
| Ubicación de contexto  | VERDE/AMARILLO/ROJO | ... |

## Exposición a modos de falla

| Modo | Exposición | Causado por |
|---|---|---|
| Poisoning (falso)              | ALTA/MEDIA/BAJA | {los hallazgos que lo alimentan} |
| Bloat (demasiado)              | ALTA/MEDIA/BAJA | ... |
| Confusion (falta o está mal)   | ALTA/MEDIA/BAJA | ... |
| Clash (contradictorio)         | ALTA/MEDIA/BAJA | ... |

## Desde la última auditoría
{solo si existe un reporte previo: qué se arregló / qué empeoró / qué es nuevo. Omitir en la primera corrida.}

## Qué haría que tu agente te responda mal hoy
{2-4 viñetas: los hallazgos que causan "dice que no está pero sí está" o respuestas confiadas pero obsoletas}

## Hallazgos por check
{viñetas concisas por check; cada hallazgo nombra una ruta concreta Y termina con su
etiqueta de modo de falla: [poisoning] [bloat] [confusion] [clash]. Incluir la tabla de
frescura de feeds: feed | fecha cruda | fecha ingerida | cadencia | veredicto | qué falta.}

## Preguntas para vos
{feeds clasificados COMO RETIRADO? — "¿lo dejaste a propósito?" — y cualquier otra cosa que solo el dueño puede responder}

## Lista de arreglos (por lotes, a la espera de aprobación)
- Lote A — seguridad + capacidades muertas (primero)
- Lote B — reconciliación de routing + índices + reubicar archivos mal puestos (limpieza de raíz)
- Lote C — ponerse al día con datos (re-correr pipelines congelados, refrescar memoria obsoleta, retirar feeds muertos oficialmente)
- Lote D — durabilidad (hooks/crons/rituales para que no se vuelva a congelar en silencio)
```

Reglas de veredicto: ROJO = al menos un hallazgo que causaría una respuesta incorrecta hoy (pipeline congelado, índice mentiroso, regla de routing muerta o mal apuntada, secreto trackeado, dato obsoleto en un archivo siempre cargado). AMARILLO = deriva que va a llegar ahí (carpetas sin indexar, duplicados, bloat, falta regla de precedencia). VERDE = revisado y limpio. Los feeds que el usuario confirma como retirados no cuentan para ROJO una vez que el OS deja de implicar que están vigentes. Sé honesto; una primera corrida sobre un AIOS real y usado rara vez debería salir todo verde.

Reglas de exposición: un modo es ALTA cuando lo alimenta un hallazgo ROJO, MEDIA cuando solo lo alimentan hallazgos AMARILLOS, BAJA cuando nada lo alimenta. La tabla de exposición es el momento pedagógico de la auditoría: le dice al usuario no solo qué está roto sino *cómo va a morder*.
