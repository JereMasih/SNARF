...
| `llm_role` | string \| null | `snarf/telemetry/context.py` (mismo patrón `threading.local()` que `conversation_id`) — seteado por `_ResilientLLM.generate()` (`llm_routing.py`) y por `Orchestrator.handle()`/`generate_conversation_title()` para sus dos roles de instancia fija, siempre limpiado en un `finally` | **Agregado en ADR de esta ronda** (2026-08-06), tras un incidente real: un prompt de 82.284 tokens tumbó el server MLX local y no había forma de saber, mirando `usage_log.jsonl`/`telemetry_events.jsonl` después, qué rol de `llm_routing.ROLES` (`"orchestrator"`, `"gmail_digest"`, `"dashboard_curator"`, etc.) lo había generado — solo vendor/modelo/tokens, nunca de dónde vino. `null` en eventos que no vienen de una llamada real a un LLM. |
